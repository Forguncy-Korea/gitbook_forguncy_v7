...
실제로 사용자는 종종 복잡한 명령 또는 명령 그룹을 정의하고 여러 곳에서 사용합니다. 활자 격자는 명령 재사용을 위한 템플릿 명령이라는 특수 명령을 제공합니다.

템플릿 명령에는 이름, 일부 매개 변수 및 사용자가 직접 정의할 수 있는 명령 목록이 포함되어 있습니다. 사용자가 템플릿 명령을 사용해야 하는 경우 명령의 "재사용"을 달성하기 위해 매개 변수에 값을 할당하기만 하면 됩니다.
...
예를 들어 "첫페이지로 이동"하는 새 템플릿 명령을 만들거나 편집 명령을 "페이지 이동 명령 만들"로 편집하고 첫 페이지로 이동합니다.&#x20;

![](<../../.gitbook/assets/image (673).png>)

다른 페이지에서는 재사 명령을 사용할 수 있습니다.

주문 목록에서 버튼 설정하고, 명령을 편집하고, 명령 "재사용 명령 호출"을 선택하고, 재사 명령을 선택합니다. 재사용 명령에 파라미터가 있는 경우 파라미터도 설정해야 합니다.

![](<../../.gitbook/assets/image (1377).png>)

실행 후 홈 페이지에서 버튼을 클릭하여 주문 목록 페이지 및 직원 목록 페이지로 이동한 다음 뒤로 버튼을 클릭하여 홈 페이지로 이동합니다.
